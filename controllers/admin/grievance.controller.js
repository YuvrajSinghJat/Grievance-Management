...
const getAllPendingGrievances = async (req, res) => {
  try {
    console.log("Hitting getAllPendingGrievances");

    const grievances = await Grievance.find({ status: "Pending" }).select("_id grievanceTitle scholarNo meetingDate meetingTime venue");
